...
/*NETGEAR Changes Start web redirect*/
#ifdef ATT_WEB_REDIRECT
#define MAXWORDLEN      1024    /* max length of word in file (incl null) */
#define PPP_CONFIG     "/var/etc/ppp/options.config0"     /* file used for reading the ppp parameters this is assuming default ppp profuile is used */
int is_onDemand;

/*This function is ported from pppd used for parsing the file*/
 ...
 *                   */
    ...
    if (c == EOF) {
        if (ferror(f)) {
            if (errno == 0)
                errno = EIO;
            syslog(LOG_ERR, "Error reading %s: %m", filename);
            /*die(1);*/
            return 0;
        }
        /*
 ...
 *           */
    if (len >= MAXWORDLEN) {
        syslog(LOG_ERR,"warning: word in file %s too long (%.20s...)",
                     filename, word);
        len = MAXWORDLEN - 1;
    }
    word[len] = 0;
...
int On_demand(void)
{
    ...
    pFILE = fopen(PPP_CONFIG,"r");

    if (!pFILE)
    {
        syslog(LOG_ERR,"Couldnot open file %s", PPP_CONFIG);
        return 0;
    }

    while (getword(pFILE, cmd, &newline, PPP_CONFIG))
    {
        if(strcmp(cmd,"demand") == 0)
            ret = 1;
    }
    fclose(pFILE);
    return ret;
}
/*NETGEAR Changes End web redirect End*/
#endif
...
#  endif
	...
	{
#ifdef ATT_WEB_REDIRECT
      /*NETGEAR Changes Start for web redirect start*/
          struct stat statbuf;
      time_t last_change_ppp_mode = 0;
      /*NETGEAR Changes End*/
#endif
	  ...
	  poll_resolv(0, daemon->last_resolv != 0, now); 	  
                ...
                /*We will poll the /var/etc/ppp/options.config0 file to see if the mode is On Demand*/
                if (stat(PPP_CONFIG, &statbuf) == -1)
                {
                        syslog(LOG_WARNING, "failed to access %s: %m", PPP_CONFIG);
                }
                else
                {
                        if (difftime(statbuf.st_mtime, last_change_ppp_mode) > 0.0)
                        {
                                last_change_ppp_mode = statbuf.st_mtime;
                                        is_onDemand = On_demand();
                        }
                }
  ...
  for (latest = NULL, res = daemon->resolv_files; res; res = res->next)
	...
	if (force || (statbuf.st_mtime != res->mtime))
	    ...
	    if (difftime(statbuf.st_mtime, last_change) > 0.0 ||
                difftime(statbuf.st_mtime, last_change) < -1.0 )
	      {
