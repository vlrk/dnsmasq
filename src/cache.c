...
struct crec *cache_find_by_name_org(struct crec *crecp, char *name, time_t now, unsigned short prot)
{
  struct crec *ans;
...
{
    struct crec *tmpcrec, *installcrec = NULL;

    /* If the hostname is "" or the IP address is 0.0.0.0 - return refused.*/
    if( (strcmp(gWBA_Host, "") == 0) || gWBA_IPAddr.s_addr == 0 )
    {
#ifdef DEBUGWESTELL
        printf(">>>>>cache.c/%s(%d): WBA not enabled\n",__FUNCTION__, __LINE__);
#endif /* DEBUGWESTELL*/
        return cache_find_by_name_org(crecp, name, now, prot);
    }

    if( strstr( name, gWBA_Domain ) == NULL )
    {
        /* If not in cache and the request is not for our local domain, then return a 
 *          dummy cache record that contains our default return.*/
#ifdef DEBUGWESTELL
        printf(">>>>>cache.c/%s(%d): Search Cache for install name: %s\n",__FUNCTION__, __LINE__, name);
#endif /*DEBUGWESTELL*/

        installcrec = cache_find_by_name_org(crecp, gWBA_Host, now, prot);

#ifdef DEBUGWESTELL
        if( installcrec )
            printf(">>>>>cache.c/%s(%d): Search Cache for install name: %s FOUND!\n",__FUNCTION__, __LINE__, name);
#endif /*DEBUGWESTELL*/
    }

    tmpcrec = cache_find_by_name_org(crecp, name, now, prot);

#ifdef DEBUGWESTELL
    if( tmpcrec )
        printf(">>>>>cache.c/%s(%d): Search Cache for requested name succeded: %s FOUND!\n",__FUNCTION__, __LINE__, name);
#endif /*DEBUGWESTELL*/

    if (tmpcrec &&
        (tmpcrec->flags & F_FORWARD) &&
        (tmpcrec->flags & prot) &&
        hostname_isequal(cache_get_name(tmpcrec), name))
    {
#ifdef DEBUGWESTELL
        printf(">>>>>cache.c/%s(%d): Return requested Answer: %s\n",__FUNCTION__, __LINE__, name);
#endif /*DEBUGWESTELL*/
        return tmpcrec;
    }
#ifdef DEBUGWESTELL
    else
        printf(">>>>>cache.c/%s(%d): %s is Not in Cache Send Install %s\n",__FUNCTION__, __LINE__, name, gWBA_Host);
#endif /*DEBUGWESTELL*/

    if (installcrec && 
        (installcrec->flags & F_FORWARD) &&
        (installcrec->flags & prot) &&
        hostname_isequal(cache_get_name(installcrec), gWBA_Host))
    {
#ifdef DEBUGWESTELL
        printf(">>>>>cache.c/%s(%d): Return Install host Answer: %s\n",__FUNCTION__, __LINE__, gWBA_Host);
#endif /*DEBUGWESTELL*/
        return installcrec;
    }
#ifdef DEBUGWESTELL
    printf(">>>>>cache.c/%s(%d): return temprec\n",__FUNCTION__, __LINE__);
#endif /*DEBUGWESTELL*/
