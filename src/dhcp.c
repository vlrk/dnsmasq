...
void dhcp_packet(time_t now, int pxe_fd)
{
 ...
 /* NETGEAR Changes Start  */
  #ifdef INCLUDE_ATT_GUI        
  {
    extern  void  netmgr_sig_send(void);
    /*DHCP lease have been sent, inform network manager*/
    netmgr_sig_send();
  }
  #endif
  /* NETGEAR Changes End */
...
/* NETGEAR Changes Start  */
#ifdef INCLUDE_ATT_GUI
#define NET_MGR_PID_FILE        "/var/run/net_mgr.pid"  /*this is also in net_mgr.h*/
#define FWCNTL_PID_FILE         "/var/run/fwcntl.pid"
static  int                                     netmgr_sig;

/*
 * **  Operate flag which requests sending a SIGUSR2 to network manager 
 * **  when a host-based DHCP lease is issued
 * */
void
netmgr_sig_request(int req)
{
        netmgr_sig = req;
}

/*
 * **  If the signal request flag is set, send SIGUSR2 to network manager
 * */
void
netmgr_sig_send(void)
{
        pid_t   netmgr_pid;
        FILE*   fd;
        char    buf[64];
        ...
        if (!netmgr_sig)
                return;
        if((fd = fopen(FWCNTL_PID_FILE, "r")))
        {
                buf[0] = 0;
                fgets(buf, sizeof(buf), fd);
                netmgr_pid = atoi(buf);
                fclose(fd);
                my_syslog(LOG_INFO,"Sending signal to the fwcntl\n");
                kill(netmgr_pid, SIGUSR2);
        }
        else
        {
                my_syslog(LOG_INFO,"No need to Send signal to the fwcntl\n");
        }



        if (!(fd = fopen(NET_MGR_PID_FILE, "r")))
                return;
